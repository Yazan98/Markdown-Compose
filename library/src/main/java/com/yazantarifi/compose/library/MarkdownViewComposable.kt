...
private fun RenderComponent(item: MarkdownComponent, config: MarkdownConfig, onLinkClickListener: (String, Int) -> Unit) {
    when (item) {
        ...
        is MarkdownLinkComponent -> MarkdownLinkComponentComposable(item.text, item.link, config.colors?.get(MarkdownConfig.LINKS_COLOR) ?: Color.Black, config.isLinksClickable, onLinkClickListener)
        is MarkdownCheckBoxComponent -> MarkdownCheckBoxComponentComposable(item.text, config.colors?.get(MarkdownConfig.CHECKBOX_COLOR) ?: Color.Black, item.isChecked)
